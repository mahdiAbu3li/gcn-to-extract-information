...
class Grapher:
    ...
    def graph_formation(self, export_graph=False):
        ...
        # df["xmin"] = df["xmin"].apply(lambda x: int(x * 1000))
        # df["ymin"] = df["ymin"].apply(lambda x: int(x * 1000))
        # df["xmax"] = df["xmax"].apply(lambda x: int(x * 1000))
        # df["ymax"] = df["ymax"].apply(lambda x: int(x * 1000))
    ...
    def relative_distance(self, export_document_graph=False):
        ...
        for index in df['index'].to_list():
            # print(index, "mahdi")
            # print(df.loc[df['index'] == index, 'right'], "imad")
